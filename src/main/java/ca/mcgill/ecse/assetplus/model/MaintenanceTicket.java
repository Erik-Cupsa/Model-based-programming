...
public class MaintenanceTicket {
    ...
    public boolean assign(HotelStaff staff, PriorityLevel aPriority, TimeEstimate aTimeToResolve, Boolean requiresApproval) {
    ...
    public boolean rejectFix(Date aDate, String aDescription) {
        boolean wasEventProcessed = false;
        ...
        switch (aStatus) {
            case Resolved:
                ...
                doRejectFix(aDate, aDescription);
                setStatus(Status.InProgress);
                wasEventProcessed = true;
                break;
            default:
    ...
    public boolean addTicketNoteAt(MaintenanceNote aTicketNote, int index) {
        ...
        if (addTicketNote(aTicketNote)) {
            ...
            if (index > numberOfTicketNotes()) {
                index = numberOfTicketNotes() - 1;
            }
            ticketNotes.remove(aTicketNote);
    ...
    public boolean addTicketImageAt(TicketImage aTicketImage, int index) {
        ...
        if (addTicketImage(aTicketImage)) {
            ...
            if (index > numberOfTicketImages()) {
                index = numberOfTicketImages() - 1;
            }
            ticketImages.remove(aTicketImage);
